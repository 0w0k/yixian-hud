...
# Reverse of _SEP_NORMALIZE: middle dot (U+00B7 ·) → bullet (U+2022 •). Used
# to emit BOTH separator spellings — card_id_map.json mixes the two, and the
# in-game CardConfig.name the HUD reads may use either.
...
class Counter:
    ...
    def deck_pool(self, player_sect: int, player_phase: int) -> dict:
        """#1 卡池补全:列出「本宗门(sect==player_sect 或通用 sect 0)+ 当前阶段
        (phase==player_phase)」的所有宗门常规牌的剩余张数 —— 含没抽到的满数牌、已删
        空的 0。用 card_phases.json 列全集,套 remaining() 同样的 max_copies/removed
        规则。机缘/灵宠(个人牌)与副职牌排除(后者仍由 remaining() 的见过逻辑覆盖)。"""
        out: dict = {}
        try:
            with _PHASE_MAP_FILE.open(encoding="utf-8") as f:
                data = json.load(f)
            ...
            return out
        personal = _personal_card_names()
        sidejob_prefix_map = _name_to_sidejob_prefix()
        # 已选副职(主职 + 各副职兼修)的 prefix 集合 —— 只有选了的副职,其牌才进牌库。
        try:
            import shadow_state as _ss
            picked_career = _ss.get_career_pick()
            sec_picks = _ss.get_secondary_career_picks()
            ...
            sec_picks = []
        picked_prefixes = set()
        _pp = CAREER_TO_SIDEJOB_PREFIX.get(picked_career, 0)
        if _pp:
            picked_prefixes.add(_pp)
        for _sp in sec_picks:
            _spp = CAREER_TO_SIDEJOB_PREFIX.get(_sp.get("career_id", 0), 0)
            if _spp:
        ...
        for cid_str, info in data.items():
            ...
            if not nm:
                continue
            name = _canonical(_normalize_sep(nm))
            if name in personal:                        # 机缘/灵宠等个人牌
                continue
            face = _normalize_sep(_PAIR_CANONICAL_TO_FACE.get(name, name))
            sidejob_prefix = sidejob_prefix_map.get(face)
            if sidejob_prefix in SIDEJOB_PREFIXES:
                # 副职牌:只有选了对应副职才在牌库;张数取已按阶段填充的副职池。
                if sidejob_prefix not in picked_prefixes:
                    continue
                max_copies = self._sidejob_pool.get(name, 0)
                if max_copies <= 0:                     # 该牌这阶段还没进副职池
                    continue
            else:
                # 宗门/中立牌:只本宗门(sect 0 是中立/机缘/灵宠杂烩,不算)。
                if info.get("sect") != player_sect:
                    continue
                if name in SPECIAL_COPIES:
                    max_copies = SPECIAL_COPIES[name]
                elif info.get("phase") == 5:
                    max_copies = PHASE5_COPIES
                else:
                    max_copies = DEFAULT_COPIES
            reroll_mult = 0 if name in NO_REROLL_PENALTY else 3
